...
void MAXreg_wr(BYTE reg, BYTE val) {
	//psuedocode:
//	xil_printf("Write Enable");
	//select MAX3421E 
	if (XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask)) {
		xil_printf ("Slave Select Error \n");
	}
	//write reg + 2 via SPI
//	BYTE temp = reg + 2;
	u8 DataBuffer[2];
	...
	DataBuffer[1] = val;
	//write val via SPI
//	XSpi_Transfer(&SpiInstance, &temp, NULL, (u8)1);
	int returncode = XSpi_Transfer(&SpiInstance, DataBuffer, NULL,2);
...
//	u8 DataBuffer[nbytes + 1];
	...
	int returncode = XSpi_Transfer(&SpiInstance, DataBuffer, NULL, nbytes + 1);
...
BYTE MAXreg_rd(BYTE reg) {
	//psuedocode:
	//select MAX3421E (
//	xil_printf("Read Enable");
	if (XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask)) {
		xil_printf ("Slave Select Error \n");
	}
//	xil_printf("Slave Captured");
//	write reg via SPI
	u8 DataBuffer[2];
	DataBuffer[0] = reg;
	DataBuffer[1] = 0;
...
//	BYTE buffer[2];
//	buffer[0] = reg;
	int returncode = XSpi_Transfer(&SpiInstance, DataBuffer, readbuffer, 2);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if (returncode != 0) {
		xil_printf ("error \n");
...
//	u8 DataBuffer[nbytes + 1];
	...
	int returncode = XSpi_Transfer(&SpiInstance, DataBuffer, readbuffer, nbytes + 1);
	if (returncode != 0) {
		xil_printf ("error \n");
	}
	for (int i = 0; i < nbytes; ++i) {
